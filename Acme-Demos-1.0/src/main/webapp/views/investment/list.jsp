...
<display:table name="investments" id="row" pagesize="5" requestURI="${requestUri}" class="displaytag">
	...
	<display:column titleKey="investment.instalments">
			<input type="button" value="<spring:message code="investment.instalments" />" 
					onclick="javascript: window.location.assign('instalment/investor/list.do?investmentId=${row.demo.id}')" />
	</display:column>
	
	<display:column titleKey="investment.instalment.create">
			<input type="button" value="<spring:message code="investment.instalment.create" />" 
					onclick="javascript: window.location.assign('instalment/investor/create.do?investmentId=${row.demo.id}')" />
	</display:column>
